...
exports.newInstance = function($, spec, cb) {
    try {
        ...
        /* Note that when this function throws, plug_pubsub will unsubscribe it,
         allowing GC */
        var deliverMsgF = function(topic, msg) {
            ...
            if (methodName) {                
                var cb0 = function(err, data) {
                    if (err) {
                        $.ca.$.log &&
                            $.ca.$.log.warn('Ignoring pubsub exception ' +
                                            myUtils.errToPrettyStr(err));
                    } else {
                        if (data) {
                            $.ca.$.log &&
                                $.ca.$.log.debug('Ignoring pubsub  value ' +
                                                 JSON.stringify(data));  
                        }
                    }
                };
                if (that.__ca_isShutdown__) {
                    ...
                    $.ca.__ca_process__(req, cb0);
                }
            } else {
                err = new Error('pubsub: Ignoring message from unsubscribed' +
                                ' channel');
                err.topic = topic;
                err.msg = msg;
                throw err;
