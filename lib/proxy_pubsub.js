...
exports.newInstance = async function($, spec) {
     ...
     *                           or
     *      async function(topic:string, value:string)
     *
     ...
     *
     * An alternative is to use an **internal** method, i.e., one that starts
     * with prefix `__ca_`. In that case **any** other CAs can call, i.e.,
     * no security ACLs, but the methods signature add an authenticated
     * `from` field:
     *
     *     __ca_whatever__(topic:string, value:string, from: string, cb: cbType)
     *                           or
     *     async __ca_whatever__(topic:string, value:string, from:string)
     *
     * Note that `this.$.security.getCallerFrom()` does not provide the real
     * caller for internal methods, and we need the extra argument.
     *
     * Internal methods are safer because requests always come from the trusted
     * bus and not from external sessions. Therefore, we know the request came
     * from a call to `publish()` in our app, and not from arbitrary code.
     *
